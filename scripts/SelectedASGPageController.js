...
SelectedASGPageControllers.controller('SelectedASGPageController', function($scope,$http,$filter,$rootScope){
  $scope.ASGNames =[];
  console.log($rootScope.selectedASG);
  ...
  $scope.DentalPartiallyAutomated = 0;
   ...
   for (var i = 0; i< $scope.RXHDApplicationName.length; i++) {
      ...
      $scope.RXHDBussinessCapabilityData.push([RXHDApplication,'RX-HD',$scope.RXHDBussinessCapability.length]);
    ...
    if($scope.RXHDBussinessCapabilityData.length==1){
      $scope.chartObject1.data = ([['ApplicationName', 'ASGName', 'BusinessCount'],
          ['RX-HD',null,0],$scope.RXHDBussinessCapabilityData[0]
        ]);
    }

    if($scope.RXHDBussinessCapabilityData.length==2){
     $scope.chartObject1.data = ([['ApplicationName', 'ASGName', 'BusinessCount'],
          ['RX-HD',null,0],$scope.RXHDBussinessCapabilityData[0],$scope.RXHDBussinessCapabilityData[1]
      ]);
    }

    if($scope.RXHDBussinessCapabilityData.length==3){
      $scope.chartObject1.data = ([['ApplicationName', 'ASGName', 'BusinessCount'],
          ['RX-HD',null,0],$scope.RXHDBussinessCapabilityData[0],$scope.RXHDBussinessCapabilityData[1],$scope.RXHDBussinessCapabilityData[2]      
      ]);
    }
...
console.log("----->"+$scope.chartObject1.data);

    function showFullTooltip(row, size, value, column) {
    return '<div style="background:#fd9; padding:10px; border-style:solid">' +
           '<span style="font-family:Courier"><b> Business Capability Count : ' + size + '</span><br>' +
           ' </div>';
  }
